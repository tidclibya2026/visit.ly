...
import { assignContentRole, assignVisaIntake, createManagedDestination, createManagedExperience, createManagedSection, createMediaAsset, createTranslationReview, createVisaIntake, getAdminOperationsReport, getContentAccess, getMonthlyPerformance, getPublishedContentItem, getTranslationDashboardMetrics, listAdminNotifications, listAssignableVisaStaff, listContentAccess, listManagedContent, listPublishedContent, listTranslationAuditLogs, listTranslationReviews, listTranslationSuggestions, listVisaIntakeHistory, listVisaIntakes, markAdminNotificationRead, permissionAllows, recordInteraction, reviewTranslation, setContentPermission, setMonthlyTarget, submitTranslationSuggestion, updateManagedDestination, updateManagedExperience, updateManagedSection, updateTranslationSuggestion, updateVisaIntakeStatus } from "./db";
...
export const appRouter = router({
  ...
  visa: router({
    ...
    assignableStaff: protectedProcedure.query(async ({ ctx }) => { await requireContentAction(ctx, "visa", "review"); return listAssignableVisaStaff(); }),
    assignIntake: protectedProcedure.input(z.object({ id: z.number().int().positive(), assigneeOpenId: z.string().min(4).max(64).nullable() })).mutation(async ({ ctx, input }) => { await requireContentAction(ctx, "visa", "review"); await assignVisaIntake(input.id, input.assigneeOpenId, ctx.user.openId); return { ok: true }; }),
    monthlyPerformance: protectedProcedure.query(async ({ ctx }) => { await requireContentAction(ctx, "visa", "review"); return getMonthlyPerformance(); }),
    setMonthlyTarget: adminProcedure.input(z.object({ monthKey: z.string().regex(/^\d{4}-\d{2}$/), visaTarget: z.number().int().min(0).max(100000), contentTarget: z.number().int().min(0).max(100000) })).mutation(async ({ ctx, input }) => { await setMonthlyTarget(input, ctx.user.openId); return { ok: true }; }),
