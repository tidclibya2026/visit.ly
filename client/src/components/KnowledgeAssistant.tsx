...
import { curatedFaqEntries } from "@/lib/assistantFaqs";
import { trpc } from "@/lib/trpc";
import { languageOptions, type SupportedLanguage, useLanguage } from "@/contexts/LanguageContext";

const topics = ["الكل", "التأشيرات", "الوجهات", "الثقافة والتراث", "الطعام", "السفر العملي"] as const;
type Topic = typeof topics[number];
type AssistantLocale = SupportedLanguage;
...
export const localizedFaqs: LocalizedFaq[] = [
  ...
  { source: "ما الأطباق الليبية التي يمكن أن أبدأ بها؟", topic: "الطعام", en: "Which Libyan dishes are a good place to start?", fr: "Quels plats libyens permettent de découvrir la cuisine locale ?", enCards: ["A good introduction includes dishes such as asida or bazin, which are prepared around grains and sauce, as well as mbakbaka and asban.", "For a sweet finish, ask about local maqrud or other regional sweets. Check ingredients and food allergies with your host or restaurant before ordering."], frCards: ["Pour commencer, découvrez des plats comme l'assida ou le bazin, préparés autour de céréales et de sauce, ainsi que la mbakbaka et l'asban.", "Pour une note sucrée, renseignez-vous sur le maqroud ou d'autres douceurs régionales. Vérifiez les ingrédients et les allergies avec votre hôte ou le restaurant avant de commander."] },
  { source: "كيف أتعرف إلى الصناعات التقليدية باحترام؟", topic: "الثقافة والتراث", en: "How can I discover traditional crafts respectfully?", fr: "Comment découvrir l'artisanat traditionnel avec respect ?", enCards: ["Look for pottery, textiles, ornaments, and handmade decoration in local markets or exhibitions, then ask about the material, making process, and story behind the piece.", "Choose handmade work from a known maker, and never buy or transport archaeological objects or items of uncertain origin."], frCards: ["Recherchez poteries, textiles, ornements et décorations faites main dans les marchés ou expositions, puis demandez le matériau, la technique et l'histoire de la pièce.", "Privilégiez le travail manuel d'un artisan identifié et n'achetez ni ne transportez d'objets archéologiques ou d'origine incertaine."] },
  { source: "ما الذي ينبغي مراعاته عند حضور مناسبة شعبية أو فرح؟", topic: "الثقافة والتراث", en: "What should I keep in mind at a popular celebration or wedding?", fr: "Que faut-il prendre en compte lors d'une célébration populaire ou d'un mariage ?", enCards: ["Popular customs, traditional dress, and weddings are social occasions with their own privacy. Follow the host's arrangements and ask before taking photographs or joining in.", "Respect local dress and customs. This makes the visit an experience of appreciation and learning rather than a passing observation."], frCards: ["Les pratiques populaires, les tenues traditionnelles et les mariages sont des occasions sociales qui ont leur intimité. Suivez les dispositions de l'hôte et demandez avant de photographier ou de participer.", "Respectez les usages et la tenue locale. La visite devient ainsi une expérience d'appréciation et de découverte, plutôt qu'une simple observation."] },
];
...
const knowledgeEntries = [...faqEntries, ...curatedFaqEntries];
...
export function KnowledgeAssistant() {
  ...
  const scopedEntries = useMemo(() => topic === "الكل" ? knowledgeEntries : knowledgeEntries.filter((entry) => topicFor(entry) === topic), [topic]);
  ...
  const localizedResults = useMemo(() => {
    ...
    return localizedFaqs.filter((item) => (topic === "الكل" || item.topic === topic) && (!query.trim() || (locale === "en" ? item.en : item.fr).toLowerCase().includes(query.trim().toLowerCase()))).map((item) => ({ item, entry: knowledgeEntries.find((entry) => normalize(entry.question) === normalize(item.source)) })).filter((item): item is { item: LocalizedFaq; entry: FaqEntry } => Boolean(item.entry));
  }, [locale, query, topic]);
  ...
  const languageStatus = locale === "ar" ? "إجابة المصدر بالعربية" : `ترجمة موجزة إلى ${languageOptions.find((item) => item.code === locale)?.label ?? locale}`;
  ...
  const choose = (entry: FaqEntry) => {
    ...
    if (localized && (locale === "en" || locale === "fr")) {
  ...
  return <aside className={`knowledge-assistant ${open ? "is-open" : ""}`} aria-label="المساعد المعرفي">
    {open && <div className="assistant-panel">
      ...
      {selected ? <div className="assistant-answer"><div className="assistant-answer-meta"><BookOpen size={15} /><span>{topicFor(selected)}</span><button type="button" onClick={() => { setSelected(null); setQuery(""); setTranslation(null); setTranslationError(""); }}>كل الأسئلة</button></div><h3>{question}</h3>{translateMutation.isPending ? <p className="assistant-translating"><Loader2 size={15} className="spin" /> جارٍ تجهيز الإجابة…</p> : <><div className="assistant-response-flow"><p className="assistant-response-lead"><Sparkles size={14} /> خلاصة عملية من دليل Visit Libya</p>{cards.map((card, index) => <p key={`${selected.question}-${index}`}>{card}</p>)}</div>{translationError && <p className="assistant-translation-note" role="status">{translationError}</p>}</>}{relatedDestinations.length > 0 && <nav className="assistant-place-links" aria-label="وجهات مرتبطة بالإجابة"><span><MapPin size={13} /> تابع الاستكشاف</span>{relatedDestinations.map((destination) => <Link href={`/destinations/${destination.id}`} key={destination.id}>{destination.title}<ArrowLeft size={13} /></Link>)}</nav>}{selected.answer.length > cards.join(" ").length && locale === "ar" && <span className="assistant-truncated">هذه خلاصة موجزة؛ اكتب اسم المدينة أو التجربة للحصول على توجيه أدق.</span>}</div> : <div className="assistant-results"><p className="assistant-label">{query ? "أسئلة مطابقة" : locale === "ar" ? `اختر سؤالًا عن ${topic === "الكل" ? "رحلتك" : topic}` : "Choose a verified travel question."}</p>{locale === "ar" || (locale !== "en" && locale !== "fr") ? (results.length ? results.map((entry) => <button type="button" key={entry.question} onClick={() => choose(entry)}><small>{topicFor(entry)}</small><span>{entry.question}</span><ChevronDown size={16} /></button>) : <p className="assistant-empty">لم أجد تطابقًا مباشرًا ضمن هذا الموضوع. جرّب اسم مدينة أو معلم أو فئة أخرى.</p>) : (localizedResults.length ? localizedResults.map(({ item, entry }) => <button type="button" key={item.source} onClick={() => choose(entry)}><small>{item.topic}</small><span>{locale === "en" ? item.en : item.fr}</span><ChevronDown size={16} /></button>) : <p className="assistant-empty">No matching translated FAQ is available in this topic. Choose another topic or return to Arabic search.</p>)}</div>}
