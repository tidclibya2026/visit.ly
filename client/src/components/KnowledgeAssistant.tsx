...
export function KnowledgeAssistant() {
  ...
  const localizedResults = useMemo(() => localizedFaqs.filter((item) => (topic === "الكل" || item.topic === topic) && (!query.trim() || (locale === "en" ? item.en : item.fr).toLowerCase().includes(query.trim().toLowerCase()))).map((item) => ({ item, entry: knowledgeEntries.find((entry) => normalize(entry.question) === normalize(item.source)) })).filter((item): item is { item: LocalizedFaq; entry: FaqEntry } => Boolean(item.entry)), [locale, query, topic]);
