          ...
          {shownDestinations.map((destination, index) => {
            ...
            return <div className="destination-record" key={destination.id}>
              <article className="catalogue-entry" data-stop={destination.id}>
                ...
                <div className="catalogue-copy"><div className="card-meta ink"><span><MapPin size={13} /> {destination.city} · {destination.region}</span><span>{destination.landmarkType}</span></div><h2><Link href={`/destinations/${destination.id}`}>{destination.title}</Link></h2><p>{destination.description}</p><div className="destination-field-note"><strong>معلومة عن الموقع</strong><span>{destination.fieldNote}</span><ul>{destination.highlights.map((item) => <li key={item}>{item}</li>)}</ul></div><span className="travel-time">الوقت المقترح: {destination.time}</span><div className="catalogue-actions"><Link href={`/destinations/${destination.id}`} className="detail-link">افتح ملف المدينة <ArrowLeft size={15} /></Link><button type="button" className="open-gallery" onClick={() => { setGalleryDestinationId(galleryOpen ? null : destination.id); setGalleryIndex(0); }} aria-expanded={galleryOpen}><Images size={16} /> {galleryOpen ? "إخفاء المعرض" : `عرض ${destination.gallery.length} صور موثقة`}</button></div></div>
