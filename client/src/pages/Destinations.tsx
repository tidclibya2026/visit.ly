  ...
  return (
    <SiteShell>
      ...
      <section className="page-frame destinations-content">
        ...
        <div className="results-line"><p>تظهر الآن <strong>{shownDestinations.length}</strong> وجهات من الدفتر.</p><label className="destination-sort">رتّب حسب<select value={sort} onChange={(event) => setSort(event.target.value as DestinationSort)}><option value="default">ترتيب الدفتر</option><option value="region">المنطقة</option><option value="category">الفئة</option></select></label><Link href="/trip">عرض مساري <ArrowLeft size={15} /></Link></div>
