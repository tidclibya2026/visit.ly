...
export default function Heritage() {
  return (
    <SiteShell>
      ...
      <section className="heritage-intro page-frame"><span className="seal-large"><Landmark size={28} /></span><div><p className="eyebrow">خمس محطات</p><h2>نوافذ على ذاكرة ليبيا.</h2></div><p>تلخّص هذه المحطات المواقع التي وردت في ملفات المنصة: مدن رومانية وكلاسيكية، عمارة الواحة، وفنون صخرية تعود إلى عصور سحيقة.</p></section>
      <section className="heritage-list page-frame">
        {heritageSites.map((site, index) => <article className={`heritage-entry entry-${index + 1}`} data-catalogue={`ملف التراث · ${String(index + 1).padStart(2, "0")}`} key={site.title}><div className="heritage-image"><img src={site.image} alt={site.alt} /></div><div className="heritage-copy"><span className="heritage-index">0{index + 1}</span><p className="eyebrow">{site.kicker}</p><h2>{site.title}</h2><p>{site.description}</p><Link href="/trip" className="underlined-link">أضف المنطقة إلى خطتك <ArrowLeft size={16} /></Link></div></article>)}
      </section>
