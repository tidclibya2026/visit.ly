    ...
    <SiteShell>
      ...
      <section className="page-frame experiences-list">
        {experiences.map((experience, index) => {
          ...
          return <article className="experience-row" key={experience.title}><span className="experience-number">0{index + 1}</span><div className="experience-photo"><img src={visual.image} alt={visual.label} /><span>{visual.label}</span><i className="experience-icon"><Icon size={22} strokeWidth={1.45} /></i></div><div><p className="experience-route">محطة التجربة · 0{index + 1}</p><h2>{experience.title}</h2><p>{experience.text}</p></div><Link href={experience.title === "مدن وحضارات" ? "/heritage" : experience.title === "مذاقات محلية" || experience.title === "حرف وتقاليد" ? "/culture" : "/destinations"} aria-label={`استكشف ${experience.title}`}><ArrowLeft size={20} /></Link></article>;
        })}
      </section>
